...
// StatusError is the interface for error with status code
...
// Param replaces placeholder in error message with value
...
// Paramf replaces placeholder with formatted value
func Paramf(k, v string, a ...any) Option {
	formatValue := fmt.Sprintf(v, a...)
	return Param(k, formatValue)
}

// Extra adds extra key-value pair to error
...
// NewByCode creates a new error with status code
...
// WrapByCode wraps an error with status code
...
// Wrapf wraps an error with format message
